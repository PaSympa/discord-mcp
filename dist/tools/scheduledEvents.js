...
exports.definitions = [
    ...
    },
    {
        ...
        description: "Create a scheduled event in a guild. Use entity_type 'VOICE' or 'STAGE_INSTANCE' with a channel_id, or 'EXTERNAL' with a location and scheduled_end_time.",
        inputSchema: {
            type: "object",
            properties: {
                guild_id: { type: "string" },
...
const ENTITY_TYPE_MAP = {
    STAGE_INSTANCE: discord_js_1.GuildScheduledEventEntityType.StageInstance,
    VOICE: discord_js_1.GuildScheduledEventEntityType.Voice,
    EXTERNAL: discord_js_1.GuildScheduledEventEntityType.External,
};
const STATUS_MAP = {
    SCHEDULED: discord_js_1.GuildScheduledEventStatus.Scheduled,
    ACTIVE: discord_js_1.GuildScheduledEventStatus.Active,
    COMPLETED: discord_js_1.GuildScheduledEventStatus.Completed,
    CANCELED: discord_js_1.GuildScheduledEventStatus.Canceled,
};
const ENTITY_TYPE_NAMES = {
    [discord_js_1.GuildScheduledEventEntityType.StageInstance]: "STAGE_INSTANCE",
    [discord_js_1.GuildScheduledEventEntityType.Voice]: "VOICE",
    [discord_js_1.GuildScheduledEventEntityType.External]: "EXTERNAL",
};
const STATUS_NAMES = {
...
async function handle(name, args) {
    switch (name) {
        ...
        case "discord_get_scheduled_event": {
            const guildId = (0, client_js_1.validateId)(args.guild_id, "guild_id");
            const eventId = (0, client_js_1.validateId)(args.event_id, "event_id");
            const guild = await client_js_1.discord.guilds.fetch(guildId);
        ...
        case "discord_create_scheduled_event": {
            ...
            if (!entityType)
                throw new Error(`Invalid entity_type: "${args.entity_type}". Must be VOICE, STAGE_INSTANCE, or EXTERNAL.`);
            if ((entityType === discord_js_1.GuildScheduledEventEntityType.Voice || entityType === discord_js_1.GuildScheduledEventEntityType.StageInstance) && !args.channel_id) {
                throw new Error("channel_id is required for VOICE or STAGE_INSTANCE events.");
            }
            if (entityType === discord_js_1.GuildScheduledEventEntityType.External && !args.location) {
                throw new Error("location is required for EXTERNAL events.");
            }
            if (entityType === discord_js_1.GuildScheduledEventEntityType.External && !args.scheduled_end_time) {
            ...
            if (args.description)
                options.description = String(args.description);
            if (args.scheduled_end_time)
                options.scheduledEndTime = String(args.scheduled_end_time);
            if (args.channel_id)
                options.channel = (0, client_js_1.validateId)(args.channel_id, "channel_id");
            if (args.location)
                options.entityMetadata = { location: String(args.location) };
            if (args.image)
                options.image = String(args.image);
        ...
        case "discord_edit_scheduled_event": {
            ...
            if (args.description !== undefined)
                options.description = String(args.description);
            if (args.scheduled_start_time)
                options.scheduledStartTime = String(args.scheduled_start_time);
            if (args.scheduled_end_time)
                options.scheduledEndTime = String(args.scheduled_end_time);
            if (args.channel_id)
                options.channel = (0, client_js_1.validateId)(args.channel_id, "channel_id");
            ...
            if (args.status) {
                ...
                if (!status)
                    throw new Error(`Invalid status: "${args.status}". Must be SCHEDULED, ACTIVE, COMPLETED, or CANCELED.`);
        ...
        case "discord_create_event_invite": {
            ...
            const url = await event.createInviteURL({
                channel: args.channel_id ? (0, client_js_1.validateId)(args.channel_id, "channel_id") : undefined,
